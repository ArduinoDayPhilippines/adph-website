import React from "react";

const Location = () => {
	return (
		<div className="container grid w-full grid-cols-1 items-center justify-between gap-6 md:grid-cols-2 md:gap-12">
			<div className="text-center md:text-left">
				<h2>
					Venue <span className="text-secondary">to be announced</span>
				</h2>
				<p className="mt-3 md:text-xl">
					We are finalizing the venue details. Location, travel tips, and onsite logistics will be shared once confirmed.
				</p>
			</div>
			<div className="rounded-3xl border border-white/10 bg-[#0d0f17] px-6 py-10 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
				<div className="flex flex-col items-center gap-3 text-center">
					<p className="text-xs uppercase tracking-[0.22em] text-white/60">Location</p>
					<p className="text-2xl font-semibold">Coming Soon</p>
					<p className="text-sm text-white/70 md:text-base">
						We are securing the best-fit space for talks, exhibits, and community meetups. Check back for the full address and map.
					</p>
				</div>
			</div>
		</div>
	);
};

export default Location;
